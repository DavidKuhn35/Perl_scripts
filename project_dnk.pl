...
open(fp1, ">perlproject_report_dnk.txt");
print fp1 "Gene Number\tGene Name\tScore1\tScore2\tAverage Score\tGene Symbol\tLinkage Group\tGenbank\n";
...
foreach $gene (@genes)
{
	...
	print "$x\n";
	$x=$x+1;
# When I had this counter as $i, I got either 2 or 3 for all the numbers.  When I changed it to $x, it # worked.  Was it because the subroutine used $i?.  That's why I put the print statements in to see what it # was doing.
	print fp1 "$x\t$gene\t$s1\t$s2\t$avg\t$symb\t$chr\t$genbank\n";
...
open (fp1, "perlproject_report_dnk.txt");
	print "This is DN Kuhn's report.\n";
while (<fp1>)
{
	print;
}
