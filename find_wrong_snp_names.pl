	...
	open (SNPDATA,$ARGV[1]) || die "Can't open EP-1 SNP chip data $!\n";
		while (<SNPDATA>)	{
			...
			if (m/^S\d+/)	{	#if line has unique SNP id in first column
				...
				my $snp = $ep1[3];
				if (($snp eq "A") || ($snp eq "C") || ($snp eq "G") || ($snp eq "T"))	{
					open (LOG, ">>wrongsnp.txt") || die "can't open log $!\n";
 					print LOG "$_\n";
				}
				my $name = $ep1[6];
...
open (POPEYE, ">>/home/david/mango/mangomap/fixit2/$pname\_$date\_rawgeno.txt") || die "Can't open $pname rawgeno out $!\n";
print POPEYE "$header\n";
...
sub parprint	{
	...
	open (PAR, ">>/home/david/mango/mangomap/fixit2/$parname\_$date\_rawgeno.txt") || die "Can't open parent printout file $!\n";
	print PAR "$header\n";
