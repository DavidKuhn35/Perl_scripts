...
my $hit_result 		= $file."_TopHit_".$timestamp."_parsed.txt";
my $noHit_result 	= $file."_noTopHit_".$timestamp."_parsed.txt";
my $summary 		= $file."_".$timestamp."_TopHit_summary.txt";
my %count_total		= ();
my %count_noHit		= ();
my %count_e_hit		= ();
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
...
sub main {
	...
	print HIT "Query_name\tQuery_length\tSubject_name\tSubject_source\tSubject_description\tSubject_length\tBest_exp\tScore\tPID\tAlign_length\tStrand\tQuery_start\tQuery_stop\tSubject_start\tSubject_stop\tframe\n";
...
sub print_blast_results{
	...
	if ($best_hsp->strand('hit')< 0){
		my $strand = "Plus/Minus";
	 	print HIT  join("\t",$result->query_name,$result->query_length,$acc,$db,$best_hit->description,$best_hit->length,$best_hsp->evalue,$best_hsp->bits,$PID,$best_hsp->length('total'),$strand,$best_hsp->start('query'),$best_hsp->end('query'),$best_hsp->end('hit'),$best_hsp->start('hit'),$best_hsp->frame('query'),$best_hsp->strand('query')). "\n";
	}else{
		my $strand = "Plus/Plus";
	 	print HIT  
	 join("\t",$result->query_name,$result->query_length,$acc,$db,$best_hit->description,$best_hit->length,$best_hsp->evalue,$best_hsp->bits,$PID,$best_hsp->length('total'),$strand,$best_hsp->start('query'),$best_hsp->end('query'),$best_hsp->start('hit'),$best_hsp->end('hit'),$best_hsp->frame('query'),$best_hsp->strand('query')). "\n";
	}
}
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
