...
#	main	get sequence names from fasta file
	my $newhead = Bio::SeqIO->new(-file => ">$ARGV[1]\_new_header.fasta", -format => 'fasta'); #create a new fasta file for output
...
#	$seqname{$fasta_name} = $fasta_name;
	...
	my $newdesc = "$scafhead{$di}"; #get the new header info from file read in above
