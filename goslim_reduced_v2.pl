...
# Version		: ver. 1.0
# History		: ver. 1.0
# Description	: reads in the GOSlim data file "ATH_GO_GOSLIM_ref_date.txt" which has GOSlim annotation but with the date converted to a number, puts most recent GOSlim annotation on 
# on a single line with place holders so it is a complete data array and prints it out
# <input>		:  			
# <output>		:    
# 			
# Dependencies	: N/A
...
#	$goslim{$data[0]}{AGI} = $data[0];
		if ($data[0]ne $lastAGI)	{
			$clastdate = 0;
			$flastdate = 0;
			$plastdate = 0;
				if (($data[7] eq "C") 	{ #identify which type of goslim term (C,F,P) and the date of the annotation
				...
				$clastdate = $data[14];
	}			elsif	(($data[7] eq "F")	{	#identify which type of goslim term (C,F,P) and the date of the annotation
	...
	}		elsif (($data[7] eq "P")	{    #identify which type of goslim term (C,F,P) and the date of the annotation
...
open	(OUTFILE, ">>XXgoslim_cpf.txt") || die "Can't print outfile $!";
