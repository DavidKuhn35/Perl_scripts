...
#open (XMER, "$ARGV[2]") || die "Can't open 201 or 401mer file $!\n";
	...
	while ($xmer = $xmerobj->next_seq){
		...
		$contig = $header;
#		print "$contig\n";
		$contig =~ s/#//;  #strip # from unique SNP identifier to make it compatible with variant report
			if (exists $snpid{$contig})	{
#		print "$contig\n";
#		print "$header\t $desc\n";
...
#								print "$snpid{$contig}\n";
								$new1 = $newpos - 1;
								substr($IUseq,$new1,1)= $snpid{$contig};
#								print "$IUseq\n";
								$mer_out = Bio::Seq->new (-seq => $IUseq, -display_id => $contig, -desc => join "\t", @xdesc); #may also use header for contigs with more than one SNP
								$mer_outIO = Bio::SeqIO->new (-file => ">>var1_2_ALL_201_out.fasta", -format => "fasta"); #creates new fasta file with IUPAC substitutions for Blast against mango CDS
